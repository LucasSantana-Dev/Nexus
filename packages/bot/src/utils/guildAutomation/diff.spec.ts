...
describe('createAutomationPlan', () => {
    ...
    it('marks commandaccess mode downgrade as protected update', () => {
        const desired = {
            ...baseManifest(),
            commandaccess: {
                grants: [
        ...
        expect(
            plan.operations.find(
                (operation) =>
                    operation.module === 'commandaccess' &&
    ...
    it('marks role permission reductions as protected updates', () => {
        const desired = baseManifest()
        desired.roles!.roles[0].permissions = '0'

        const actual = baseManifest()
        actual.roles!.roles[0].permissions = '8'
        ...
        expect(
            plan.operations.find(
                (operation) =>
                    operation.module === 'roles' &&
                    operation.target.startsWith('roles/') &&
                    operation.action === 'update',
            ),
        ).toEqual(
            expect.objectContaining({
